...
for i in range(vehicle_num + size_order, (size_order*2 + vehicle_num)):
    time_window1[i][0] = 0
    time_window1[i][1] = np.random.randint(40, 101) + 10   # 加一些slack 30min 防止没有可行解 # deadline_d
...
# print(len(time_windows)) # 55 = 15+20+20 # 75 = 15+30+30
...
def print_solution(data, manager, routing, solution):
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        while not routing.IsEnd(index):
            node_index = manager.IndexToNode(index) #该路径Start节点的编号
...
#     for vehicle_id in range(data['num_vehicles']):
#         index = routing.Start(vehicle_id)
#         plan_output1 = 'Route for vehicle {}:\n'.format(vehicle_id)
#         route_distance = 0
#         route_load = 0
#         while not routing.IsEnd(index):
#             node_index = manager.IndexToNode(index) #该路径Start节点的编号
#             route_load += data['demands'][node_index]
#             plan_output1 += ' {0} Load({1}) -> '.format(node_index, route_load)
#             previous_index = index
#             index = solution.Value(routing.NextVar(index))
#             route_distance += routing.GetArcCostForVehicle(
#                 previous_index, index, vehicle_id)  # SetArcCostEvaluatorOfVehicle
#         plan_output1 += ' {0} load({1})\n'.format(manager.IndexToNode(index),
#                                                  route_load)
#         plan_output1 += 'Distance of the route: {}m\n'.format(route_distance)
#         plan_output1 += 'Load of the route: {}\n'.format(route_load)
#         print(plan_output1)
#     for vehicle_id in range(data['num_vehicles']):
#         index = routing.Start(vehicle_id)
...
#         while not routing.IsEnd(index):
#             time_var = time_dimension.CumulVar(index)
#             plan_output2 += '{0} Time({1},{2}) -> '.format(
#                 manager.IndexToNode(index), solution.Min(time_var),
#                 solution.Max(time_var))
#             index = solution.Value(routing.NextVar(index))
#         time_var = time_dimension.CumulVar(index)
#         plan_output2 += '{0} Time({1},{2})\n'.format(manager.IndexToNode(index),
#                                                     solution.Min(time_var),
#                                                     solution.Max(time_var))
#         plan_output2 += 'Time of the route: {}min\n'.format(
#             solution.Min(time_var))
#         print(plan_output2)
#         total_time += solution.Min(time_var)
#         total_distance += route_distance
...
# def print_solution(data, manager, routing, solution):
#     """Prints solution on console with both load and time routes"""
#     print(f'Objective: {solution.ObjectiveValue()}')
#     total_distance = 0
#     total_load = 0
#     # another:
...
def main():
    ...
    for vehicle_i in range(len(vehicle_cost_map)):
        def distance_callback(from_index, to_index):  # cost_callback
            ...
            return data['distance_matrix'][from_node][to_node] * vehicle_cost
        transit_callback_index = routing.RegisterTransitCallback(distance_callback)
        # cost_callback_indices.append(transit_callback_index)
        routing.SetArcCostEvaluatorOfVehicle(transit_callback_index, vehicle_i)
    # ends here)
    # print("cost_callback_indices:", cost_callback_indices)
    ...
    routing.AddDimensionWithVehicleCapacity(
        ...
        'Capacity')
    # AddDimensionWithVehicleCapacity函数接受array形式的capacity 即每辆车可以不同
    ...
    routing.AddDimension(
        ...
        0,  # no slack
        2000,  # vehicle maximum travel distance 感觉这个距离上限在本问题中居然真的有限制路径的方案，因为我们的输出结果普遍是2-3km/条
        True,  # start cumul to zero
        dimension_name)
    ...
    transitTime_callback_index = routing.RegisterTransitCallback(time_callback)
    ...
    for vehicle_id in range(data['num_vehicles']):
        index = routing.Start(vehicle_id)  # 起点序号
        time_dimension.CumulVar(index).SetRange(
            data['time_windows'][vehicle_id][0],
            data['time_windows'][vehicle_id][1])
    # 因为我们的starts列表刚好和vehicle序号是一一对应的，所以只需要对每个车辆的start时间窗设置成对应序号的depot时间窗即可
    ...
    search_parameters.time_limit.FromSeconds(5)  # 1 # 尝试增加时间限制或者改变搜索策略
